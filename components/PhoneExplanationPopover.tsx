...
export default function PhoneExplanationPopover() {
  return (
    <Popover>
      <PopoverButton className="text-xs font-semibold text-slate-400 focus:outline-hidden data-active:text-black data-focus:outline-1 data-focus:outline-white data-hover:text-slate-500">
        Why is your number needed?
      </PopoverButton>
      <PopoverPanel
        transition
        anchor="bottom"
        className="bg-opacity-80 z-10 divide-y divide-white/5 rounded-xl bg-white text-sm/6 shadow-lg backdrop-blur-xl transition duration-200 ease-in-out [--anchor-gap:var(--spacing-5)] data-closed:-translate-y-1 data-closed:opacity-0"
      >
        <div className="p-3">
